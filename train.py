...
count_max = 50
size = 4
fn_haar = 'haarcascade_frontalface_default.xml' #path
fn_dir = 'att_faces'
try:
    fn_name = sys.argv[1]
except:
...
if not os.path.isdir(path):
    os.mkdir(path)
(im_width, im_height) = (112, 92) #size
haar_cascade = cv2.CascadeClassifier(fn_haar)
webcam = cv2.VideoCapture(0, cv2.CAP_DSHOW)
#webcam = cv2.VideoCapture(0)

# Generate name for image file in num format 123..  
...
while count < count_max:
    ...
    rval = False
    while(not rval):
        # Put the image from the webcam into 'frame'-frame correct or not
